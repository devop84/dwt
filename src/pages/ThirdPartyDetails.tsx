...
export function ThirdPartyDetails() {
  ...
  const loadThirdParty = async () => {
    try {
      setLoading(true)
      setError(null)
      const data = await thirdPartiesApi.getById(id!)
      setThirdParty(data)
    } catch (err: any) {
      setError(err.response?.data?.message || err.message || 'Failed to load third party')
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true)
      await thirdPartiesApi.delete(thirdParty.id)
      navigate('/third-parties')
    } catch (err: any) {
      alert(err.response?.data?.message || err.message || 'Failed to delete third party')
    } finally {
      setDeleting(false)
    }
  }

  const handleSave = async (thirdPartyData: Omit<ThirdParty, 'id' | 'createdAt' | 'updatedAt'>) => {
    if (!thirdParty) return
    await thirdPartiesApi.update(thirdParty.id, thirdPartyData)
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <p style={{ color: '#6b7280', margin: 0 }}>Loading third party details...</p>
        </div>
      </div>
    )
  }
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ margin: 0 }}>Error: {error || 'Third party not found'}</p>
          </div>
          <button
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <span style={{ fontWeight: '600' }}>Created:</span> {formatDateTime(thirdParty.createdAt)}
          </div>
          <div>
            <span style={{ fontWeight: '600' }}>Last Updated:</span> {formatDateTime(thirdParty.updatedAt)}
          </div>
        </div>
      </div>
