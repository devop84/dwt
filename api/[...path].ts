...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    // Auth routes are handled by explicit files: api/auth/login.ts, api/auth/register.ts, api/auth/me.ts
    // Skip auth routes in catch-all to avoid conflicts
    if (route === 'auth') {
      res.status(404).json({ message: 'Auth route not found - should be handled by explicit route files' })
      return
    }
